(function(){
  ...
  let queueIndex=0;
  // Pick a different opening track on each full page refresh.
  const initialIndex = ALL.length ? Math.floor(Math.random() * ALL.length) : 0;
  let shuffle=false;
  let repeat=false;
  let player=null;
  let ytReady=false;
    ...
    player=new YT.Player(ytHost,{
      ...
      events:{
        onReady:()=>{
          ...
          // Start the randomly selected track. Browser autoplay policy may still require a user gesture.
          load(start,true);
  ...
  function toggleFullscreen(){
    const frame=document.querySelector('.hero-youtube__frame');
    ...
    if(request){
      Promise.resolve(request.call(target)).catch(()=>{
        // Fallback: try the containing frame if iframe fullscreen is refused.
        const fallback=frame?.requestFullscreen || frame?.webkitRequestFullscreen || frame?.msRequestFullscreen;
        if(fallback) Promise.resolve(fallback.call(frame)).catch(()=>{});
      });
    }
  }

  $('ytFullscreen').onclick=toggleFullscreen;
  document.addEventListener('fullscreenchange',()=>{
    const btn=$('ytFullscreen');
    if(!btn) return;
    const active=!!document.fullscreenElement;
    btn.textContent=active?'⛶':'⛶';
    btn.setAttribute('aria-label',active?'Exit fullscreen':'Enter fullscreen');
    btn.title=active?'Exit fullscreen':'Fullscreen video';
  });
